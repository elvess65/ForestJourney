...
public class UltimateJoystick : MonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
{
	...
	public void RegisterJoystick ( string newName )
	{
		if( UltimateJoysticks.ContainsKey( joystickName ) )
			UltimateJoysticks.Remove( joystickName );

		joystickName = newName;

		if( UltimateJoysticks.ContainsKey( newName ) )
			UltimateJoysticks.Remove( newName );
		
		UltimateJoysticks.Add( newName, GetComponent<UltimateJoystick>() );
	}
